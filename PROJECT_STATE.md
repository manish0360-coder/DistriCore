...
| M8 | Mobile app | **In progress.** Phase 1 signed — `docs/M8_Design_Review.md` **v1.13.0**, both ADRs approved: **Drift over an encrypted SQLite** (§5.6 — the cipher clause amended to SQLite3MultipleCiphers by **D-M9-8**, 2026-08-25; the ADR's conclusion, Drift, is unchanged) and **Dio** (§7.5). Principles P-1…P-10 frozen. **Phase 2 tasks 0–5, 7 and 9 committed** plus TD-39 (see the commit history below). **Task 8 — Owner Companion Mode — built 2026-09-06** (§3.4; OI-7 ruled with it). **Task 6 has no commit** — GPS and the separate media queue (**FR-FUL-008/011/014 are `Rel = v1.1`, so not V1** — `02` §111). **Task 10's soak harness built and committed 2026-09-14** (`integration_test/offline_soak_test.dart`, `make mobile-device-soak`, `M8_Design_Review` §5.6.3) — **the authoritative physical-device run has not happened; NFR-OFF-001 and TD-45 remain open.** `mobile/lib/features/` contains `auth`, `companion`, `customers`, `deliveries`, `settings` and `sync_status` |
...
| 2 | **FR-SYN-009 — `SALESMGR`/`ADMIN` per-device view.** | **CLOSED 2026-09-14 — ruled `Rel = v2.0` by D-M9-7 (2026-08-21, v1.2.0), transcribed into `02` as S-6 and into `03` §11.3 / `05` §11.5 the same day this row was corrected. Not built in Edition 1, and not an Edition-1 obligation.** |
| 3 | **Orphaned `RECEIVED` recovery.** *Superseded — this row quoted `05` §11.5's pre-fix clause,* **"not specified and is not built … Deferred to M9.4/M10 by ruling,"** *which `05` §11.5 itself has labelled "superseded" since the 2026-08-21 replay-on-resend fix; this row was simply never updated after that.* | **CLOSED 2026-09-14 — D-M9-13** (`docs/M9_Design_Review.md` §6). **Replay-on-resend (`05` §11.5, fixed 2026-08-21, proven 11/11 by `test_sync_integrity.py`) is the complete V1 mechanism.** A device that resends recovers cleanly; a device that never reconnects leaves an alarm (`pending_count`), not a loss. **No `02`/`04` requirement names a proactive sweep** (checked directly), so none is a V1 obligation. No code, API or schema change |
| 4 | **FR-SYN-007 remainder / stock.** | **CLOSED 2026-09-14 — deliberately deferred, not built.** Architect ruling, recorded as **D-M9-11** (`docs/M9_Design_Review.md` §6): the stock-snapshot clause moves to `Rel = v2.0` (`02` S-8). Reason: DV-1 (`02A` §5) removed field order capture from Edition 1, so no V1 mobile workflow needs a device-side stock snapshot — the clause has no Edition-1 consumer. Customers, products, prices and schemes in the same requirement are unaffected and remain `v1.0`. `05` D-M9.4-1's CONTRACT GAP is closed by this deferral, not by an implementation |
| 5 | **FR-SYN-005 / 013 / 014 — conflict console.** | **CLOSED, and found already resolved 2026-09-14.** `02` was amended with S-1…S-4 on 2026-08-21 (`M9_Design_Review` §6, D-M9-2/3/4) — this row was simply never updated after that. FR-SYN-005 stays `M`/`v1.0`, discharged by `sync_operation.status='REJECTED'`; FR-SYN-013/014 moved to `v2.0`; no `SyncConflict` table is required. `05` §11.4 already stated the correct conclusion. No corpus contradiction remains |
...
| **TD-47** | **FR-SYN-010's scope in the background is unspecified, and no in-process mechanism can satisfy it there.** `02` FR-SYN-010 said *"within 2 minutes of reconnection"* and named no application state. Android Doze and App Standby suspend in-process timers, and on many OEM builds connectivity callbacks too — so neither the shipped cadence nor a `connectivity_plus` listener could guarantee the bound while the app is backgrounded. TD-41 deliberately does **not** cancel the cadence on `paused`/`inactive`: whether a Dart timer survives backgrounding is the OS's decision, and stopping it in our own code would add a *second, self-inflicted* reason to miss the bound. **Ruled 2026-09-14 — D-M9-12** (`docs/M9_Design_Review.md` §6): the bound is **foreground-scoped for Edition 1/V1** — it applies with the app foregrounded and connectivity restored; background/Doze sync is best-effort and not an acceptance criterion. **No `WorkManager`/`JobScheduler` is added for V1.** B1's existing foreground measurement (PASSED 2026-09-06) is the complete V1 acceptance evidence; `02` amended as S-9 (`02` FR-SYN-010, NFR-PER-004) | **RULED — D-M9-12 (2026-09-14)** |
...
| **TD-45** | **`arm64` and physical hardware are unproven.** The encryption gate (D-M9-8) covers **`android-x64` on a Pixel 8a API 34 emulator only**. The same is true of every device gate run on that AVD, including B1 and the kill/storage pair. **The task 10 soak's authoritative run (`M8_Design_Review` §5.6.3) is required to be on a physical device** — the first opportunity to close this alongside NFR-OFF-001, not a separate exercise | M10 |
